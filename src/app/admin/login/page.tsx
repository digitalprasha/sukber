...
function FloatingOrbs() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <div className="absolute top-1/4 -left-20 w-72 h-72 rounded-full bg-emerald-500/10 blur-[100px] animate-pulse" />
      <div className="absolute bottom-1/3 -right-20 w-96 h-96 rounded-full bg-amber-500/8 blur-[120px] animate-pulse" style={{ animationDelay: '2s' }} />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full bg-teal-500/5 blur-[80px] animate-pulse" style={{ animationDelay: '4s' }} />
    </div>
  )
}
...
export default function AdminLoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${location.origin}/api/auth/callback?next=/admin`,
      },
    })
  }
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#060a08]">
      <FloatingOrbs />

      <div className="absolute inset-0 opacity-[0.03]">
        <div className="absolute inset-0" style={{
          backgroundImage: 'radial-gradient(circle at 25% 25%, rgba(5,150,105,0.3) 0%, transparent 50%), radial-gradient(circle at 75% 75%, rgba(245,158,11,0.2) 0%, transparent 50%)',
        }} />
      </div>
      ...
      >
        <div className="relative rounded-3xl border border-white/[0.06] bg-[#0a1a12]/80 backdrop-blur-2xl p-8 shadow-2xl shadow-emerald-500/5 overflow-hidden">
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="px-2 bg-[#0a1a12]/80 text-gray-500">atau</span>
                </div>
              </div>

              <Button
                onClick={() => setMode('password')}
                className="w-full h-12 text-base bg-white/5 hover:bg-white/10 text-white border border-white/10"
                variant="secondary"
              >
            ...
            <form onSubmit={handlePasswordLogin} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="admin@sukabernyanyi.my.id"
