...
export default function AdminGalleryPage() {
  ...
  async function loadGallery() {
    setLoading(true)
    const { data, count } = await supabase.from('gallery').select('*', { count: 'exact' })
  ...
  async function handleDelete() {
    ...
    await supabase.from('audit_logs').insert({
      ...
      details: `Menghapus galeri: ${deleteTarget.caption}`,
    })
  ...
  return (
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-2xl bg-white/5 border border-white/10 p-6 mb-6 space-y-4">
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => setForm({ ...form, type: 'youtube' })}
              className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${form.type === 'youtube' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' : 'bg-white/5 text-gray-400 border border-white/10'}`}>
      ...
      <ConfirmModal
        ...
        title="Hapus Galeri"
        message={`Yakin ingin menghapus item galeri ini?`}
        confirmText="Hapus"
        variant="danger"
      />
    </div>
